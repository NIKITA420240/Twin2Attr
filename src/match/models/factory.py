...
def build_predictor(
    ...
    if predictor_name in {"transformer", "fusion", "cascade", "stacking"}:
        from .transformer.factory import build_transformer_predictor

    transformer: TransformerPredictor | None = None
    if predictor_name in {"transformer", "fusion"}:
        transformer = build_transformer_predictor(
            solution,
            solution_root,
            usage="encoder" if predictor_name == "fusion" else "classifier",
        )
    ...
    if predictor_name == "cascade":
        ...
        main_model = build_transformer_predictor(
            solution,
            solution_root,
            usage="classifier",
        )
