...
Подготовьте Docker-контейнер и код с весами ваших обученных моделей;
...
Главное — в архиве должен быть файл metadata.json с указанием используемого докер-образа (рекомендуем начать с odsai/ecup26-matching-baseline:1.0) и строки для запуска вашей модели:
...
Docker: решение должно успешно автономно отработать на закрытых тестовых данных, уложиться по времени и доступным ресурсам, и в результате сохранить .csv файлы с результатами классификации пар;
Result: сохраненные результаты классификации пар должны соответствовать заданному формату и быть предоставлены для всех тестовых пар товаров;
Подсчет метрики: если решение прошло все проверки и нигде не упало, на результатах классификации считается метрика соревнования.
Docker-стадия
