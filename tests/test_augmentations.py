...
from match.augmentations import (
...
class AttributeWordDropoutTests(unittest.TestCase):
    def test_pair_probability_zero_is_exact_noop(self) -> None:
        pair = _pair()
        settings = AttributeWordDropoutSettings(
            pair_probability=0.0,
            attribute_dropout_probability=1.0,
            word_dropout_probability=1.0,
            keyboard_typo_probability=1.0,
            word_shuffle_probability=1.0,
            seed=42,
        )

        self.assertIs(AttributeWordDropoutAugmenter(settings)(pair), pair)

    def test_attribute_dropout_preserves_names_and_category(self) -> None:
        pair = _pair()
        settings = AttributeWordDropoutSettings(
        ...
        augmented = AttributeWordDropoutAugmenter(settings)(pair)

        self.assertEqual(augmented.category, pair.category)
        self.assertEqual(augmented.left.name, pair.left.name)
        self.assertEqual(augmented.right.name, pair.right.name)
        self.assertEqual(augmented.left.attributes, ())
        self.assertEqual(augmented.right.attributes, ())

    def test_seed_reproduces_dynamic_sequence(self) -> None:
        pair = _pair()
        settings = AttributeWordDropoutSettings(
            pair_probability=1.0,
            attribute_dropout_probability=0.5,
            word_dropout_probability=0.0,
            keyboard_typo_probability=0.0,
            word_shuffle_probability=0.0,
            seed=42,
        )
        first = AttributeWordDropoutAugmenter(settings)
        second = AttributeWordDropoutAugmenter(settings)

        first_sequence = [first(pair) for _ in range(5)]
        second_sequence = [second(pair) for _ in range(5)]

        self.assertEqual(first_sequence, second_sequence)
        self.assertGreater(len(set(first_sequence)), 1)
